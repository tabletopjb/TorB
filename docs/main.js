...
function render(){
  ...
  for (const _ of state.cpu.items){
    ui.cpuItems.appendChild(makeItemEl({type:"아이템(비공개)", desc:"상대는 확인 불가"}, true));
  }
...
function onClickPlayerItem(index){
  ...
  if (t === "서리") {
    ...
    const returned = state.player.items.length; // 되돌린 개수
    state.itemDeck.push(...state.player.items);

    // 3) 손 아이템 비우기
    state.player.items = [];

    // 4) 더미 셔플
    shuffle(state.itemDeck);

    // 5) 되돌린 개수만큼 새로 뽑기
    dealItemTo("player", returned);

    ui.hint.textContent = `서리 사용: 아이템 ${returned}장을 되돌리고 새로 뽑았습니다.`;
    render();
    return;
  }
...
/* ===== 아이템 공통 ===== */
function removeItem(side, idx){ state[side].items.splice(idx,1); }
...
async function startGame(){
  ...
  render();
  await showAlert("라운드 시작", `라운드 1이 시작되었습니다.\n내 턴입니다.`);
  ui.hint.textContent = "내 턴입니다.";
  render();
}
...
async function nextRound(){
  ...
  render();
  await showAlert("라운드 시작", `라운드 ${state.round}이 시작되었습니다.\n내 턴입니다.`);
  ui.hint.textContent = "내 턴입니다.";
  render();
}
...
function consumeItem(sideObj, type){
  ...
  if (idx >= 0) sideObj.items.splice(idx,1);
}
...
function cpuMaybeUseItem(){
  ...
  if (c.items.length >= 6 && hasItem(c, "서리") && Math.random() < 0.30) {
    ...
    // 2) 남아있는 모든 아이템을 더미로 되돌리기
    const returned = c.items.length; // 되돌린 개수
    state.itemDeck.push(...c.items);
